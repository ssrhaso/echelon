...
def checkpoint_artifact_name():
    """Per-run W&B artifact collection name for this run's best checkpoint.

    Every run MUST get its own durable collection: a shared collection lets W&B
    age out old versions and lose checkpoints. A nameless run falls back to its
    unique run id, never to a shared collection.
    """
    if wandb.run is None:
        raise RuntimeError("checkpoint_artifact_name() called with no active wandb run")
    name = wandb.run.name or ""
    # W&B artifact names allow [a-zA-Z0-9_.-] only; flatten the run path to dashes.
    safe = "".join(c if (c.isalnum() or c in "_.-") else "-" for c in name).strip("-")
    if not safe:
        safe = "run-{}".format(wandb.run.id)
        print("WARNING: run has no name; using per-run artifact collection {!r}".format(safe))
    return safe
...
class Model(modules.Module):
 ...
 - backward
    ...
    def log_step(self, losses, metrics, infos, writer, step, tag):
        ...
        # Losses
        for key, value in losses.items():
            writer.add_scalar(metric_key(tag, key), value, step)
        ...
        for key, value in infos.items():
            if isinstance(value, float) or isinstance(value, int):
                writer.add_scalar(metric_key(tag, key), float(value), step)
    ...
    def fit(
        ...
        if wandb.run is not None:
            ...
            try:
                ...
                parsed_override = None  # keep the raw string; don't fail a run over it

            wandb.config.update({
                "precision": str(precision),
                "override_config": raw_override,
                "override_config_parsed": parsed_override,
            ...
            print("provenance: precision={} host={} gpu={} override_config={!r}".format(
                precision, socket.gethostname(),
    ...
    def _evaluate(self, dataset, writer, eval_steps=None, verbose=0, recompute_metrics=False, tag="Evaluation", verbose_progress_bar=1):
        ...
        if dataset is not None:
            ...
            for dataset_i, dataset in enumerate(dataset):
                ...
                # Log
                if writer is not None:
                    self.log_step(losses=val_losses, metrics=val_metrics, infos={}, writer=writer, step=self.model_step, tag=metric_key(tag, dataset_i))
