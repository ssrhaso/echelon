...
def test_3_encoder_shape():
    ...
    assert out["pre_vq_features"].shape == (2, 4, 4096), f"pre_vq_features: {out['pre_vq_features'].shape}"
...
def test_17_transfer_all_selection():
    ...
    def build(num_actions):
        m = nn.Module()
        m.encoder_network = nn.Module()
        m.encoder_network.cnn = nn.Linear(8, 8)
        m.rssm = nn.Module()
        m.rssm.action_mixer = nn.Linear(4 + num_actions, 8)   # action-conditioned
        m.rssm.blocks = nn.Linear(8, 8)                        # task-agnostic
        m.decoder_network = nn.Linear(8, 8)
        m.value_network = nn.Linear(8, 1)
        m.policy_network = nn.Linear(8, num_actions)           # action-conditioned
        m.contrastive_network = nn.Linear(8 + num_actions, 4)  # action-conditioned
        m.register_buffer("codebook", torch.randn(16, 4))
        return m

    source, target = build(6), build(9)
    src_state = {k: v.clone() for k, v in source.state_dict().items()}

    summary = load_and_transfer_all(target, "<stub>", src_state)

    # Task-agnostic weights moved, action-conditioned ones did not.
    assert torch.equal(target.encoder_network.cnn.weight, source.encoder_network.cnn.weight)
    assert torch.equal(target.rssm.blocks.weight, source.rssm.blocks.weight)
    assert torch.equal(target.value_network.weight, source.value_network.weight)
    assert torch.equal(target.codebook, source.codebook), "codebook buffer must transfer"
    assert not torch.equal(target.policy_network.bias, source.policy_network.bias[:6]), \
        "policy head must stay at init"

    reinit = set(summary["reinit"])
    assert reinit == {"rssm.action_mixer.weight", "rssm.action_mixer.bias",
                      "policy_network.weight", "policy_network.bias",
                      "contrastive_network.weight", "contrastive_network.bias"}, reinit
    assert not summary["missing_buffers"], summary["missing_buffers"]
    assert len(summary["transferred"]) == 9, summary["transferred"]

    # Same action count must give the SAME re-init set: uniform across targets,
    # so a 6-action target is not silently a different condition from a 9-action one.
    same = build(6)
    summary_same = load_and_transfer_all(same, "<stub>", src_state)
    assert set(summary_same["reinit"]) == reinit
    ...
    try:
        load_and_transfer_all(wrong, "<stub>", src_state)
    except RuntimeError as e:
        assert "decoder_network" in str(e)
    else:
        raise AssertionError("shape mismatch outside action-conditioned modules must raise")

    # A source missing a target parameter is likewise an error.
    truncated = {k: v for k, v in src_state.items() if k != "value_network.weight"}
    try:
        load_and_transfer_all(build(9), "<stub>", truncated)
    except KeyError as e:
        assert "value_network.weight" in str(e)
    else:
        raise AssertionError("missing source parameter must raise")
...
if __name__ == "__main__":
    ...
    test_17_transfer_all_selection()
    print("\nAll 17 tests passed.")
