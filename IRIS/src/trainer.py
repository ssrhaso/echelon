...
class Trainer:
    ...
    def eval_component(self, component: nn.Module, batch_num_samples: int, sequence_length: int, **kwargs_loss: Any) -> Dict[str, float]:
        ...
        # PORT: per-level codebook liveness -- the Stage-2 cascade-collapse gate.
        # Computed at eval only, on a fresh batch, and merged STRAIGHT into the
        # logged metrics dict -- deliberately NOT routed through
        # LossWithIntermediateLosses, whose loss_total sums its kwargs and would
        # backprop these scalars. Surfaces tokenizer/eval/{usage,perplexity}_{0,1,2}
        # so L1/L2 collapse (perplexity -> 1) is visible live on W&B.
        if str(component) == 'tokenizer':
            batch = self._to_device(self.test_dataset.sample_batch(batch_num_samples, sequence_length=1))
            obs = component.preprocess_input(batch['observations'].flatten(0, 1))  # (b t) c h w
            enc = component.encode(obs, should_preprocess=False)
            for k, v in component.hrvq.get_codebook_usage(enc.tokens_levels).items():
                metrics[f'{str(component)}/eval/{k}'] = v
