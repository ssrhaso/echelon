...
class WorldModelEnv:
    ...
    def decode_obs_tokens(self) -> List[Image.Image]:
        # PORT: sum level embeddings via the residual quantiser instead of the
        # stock single self.tokenizer.embedding lookup. The world model predicts
        # only the level-0 token stream, so this sums over level 0 here; shape is
        # identical to the stock single-level decode (4x4 grid -> E channels).
        z = self.tokenizer.tokens_to_z(self.obs_tokens)                 # (B, E, h, w)
